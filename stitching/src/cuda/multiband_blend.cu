...
# ifndef SUPPRESS_DREAMVU_CHANGES


        __global__ void addSrcWeight1(const PtrStep<short> src, const PtrStepf src_weight,
                                              PtrStep<short> dst, PtrStepf dst_weight, int rows, int cols)
        {
            int x = blockIdx.x * blockDim.x + threadIdx.x;
            int y = blockIdx.y * blockDim.y + threadIdx.y;
        ...
        __global__ void addSrcWeight2(const PtrStep<short> src, const PtrStepf src_weight,
                                      PtrStep<short> dst, PtrStepf dst_weight, int rows, int cols)
        {
            int x = blockIdx.x * blockDim.x + threadIdx.x;
            int y = blockIdx.y * blockDim.y + threadIdx.y;
                                ...
                                PtrStep<short> dst, PtrStepf dst_weight, cv::Rect &rc)
        {
            dim3 threads(16, 16);
            dim3 grid(divUp(rc.width, threads.x), divUp(rc.height, threads.y));
            addSrcWeight1 << <grid, threads >> >(src, src_weight, dst, dst_weight, rc.height, rc.width);
            cudaSafeCall(cudaGetLastError());
        }
                                ...
                                PtrStep<short> dst, PtrStepf dst_weight, cv::Rect &rc)
        {
            dim3 threads(16, 16);
            dim3 grid(divUp(rc.width, threads.x), divUp(rc.height, threads.y));
            addSrcWeight2 << <grid, threads >> >(src, src_weight, dst, dst_weight, rc.height, rc.width);
            cudaSafeCall(cudaGetLastError());
        }
# endif
